...
//	_7SEG_GPIO_Init();
	...
	while (1) {
//		__HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, ccr);  // mecro
//		TIM4->CCR1 = ccr; //
//		ccr += 1000;
//
//		if(ccr > TIM4->ARR){
...
//		HAL_Delay(50);
		...
		/*use ARR ===> !!!Error!!!(Output stops during operation)
		 * Default : prescaler = 168-1, ARR = 1000-1*/
//		if (ud_flag == 0) {
//			arr++;
//			if (arr >= 2500)
//				ud_flag = 1;
...
//		HAL_Delay(1);
		/*use PSC
		 * Default : prescaler = 1000-1, ARR = 168-1*/
		if (ud_flag == 0) {
			psc++;
			if (psc >= 2500)
				ud_flag = 1;
